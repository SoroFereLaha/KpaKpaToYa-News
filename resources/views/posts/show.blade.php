...
<head>
    ...
    <title>{{ config('app.name') . ' | ' . $post->titre }}</title>
...
<body class="antialiased pt-10 pb-16 md:pb-32">
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <header class="flex justify-between items-center space-x-5 text-slate-900">
            {{-- Logo --}}
            {{-- c'est pour ca qu'on a nommé notre route index, pour l'utilisé avec la fonction route --}}
            <a href="{{ route('index') }}">
        ...
        <main class="mt-10 md:mt-12 lg:mt-16">
            <div class="space-y-10 md:space-y-16">
                ...
                <article class="flex flex-col lg:flex-row pb-10 md:pb-16 border-b">
                    <div class="lg:w-5/12">
                        <img class="w-full max-h-72 object-cover lg:max-h-none lg:h-full" src="{{ $post->lienImage }}">
                    </div>
                    <div class="flex flex-col items-start mt-5 space-y-5 lg:w-7/12 lg:mt-0 lg:ml-12">
                        ...
                        <h1 class="font-bold text-slate-900 text-3xl lg:text-5xl leading-tight">{{ $post->titre }}</h1>
